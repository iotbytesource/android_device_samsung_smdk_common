...
void AudioHardware::setVoiceVolume_l(float volume)
{
	...
	mVoiceVol = volume;
        if(AudioSystem::MODE_IN_CALL == mMode){
            ...
            case AudioSystem::DEVICE_OUT_WIRED_HEADSET:
            case AudioSystem::DEVICE_OUT_WIRED_HEADPHONE: // Use receive path with 3 pole headset.
                LOGD("### earpiece or headset call volume");
                strcpy(ctlname,"HP Volume");
                break;

            case AudioSystem::DEVICE_OUT_SPEAKER:
                LOGD("### speaker call volume");
                strcpy(ctlname,"SPK Volume");
                break;
            ...
            TRACE_DRIVER_IN(DRV_MIXER_SEL)
            mixer_ctl_set_value(ctl, 0, int_volume);
            ...
            for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt){
                ...
                if (NULL != ctl) {
                    ...
                    for (unsigned int idx = 0 ; mixer_ctl_get_num_values(ctl) > idx ; ++idx)
                        mixer_ctl_set_value(ctl, idx, mixer[cnt].val);
                    TRACE_DRIVER_OUT
                    LOGE("[wenpin]INCALL_PATH %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
                } else {
                    LOGE("failed get mixer control");
                }
            }
...
const AudioMixer *AudioHardware::getOutputRouteFromDevice(uint32_t device)
{
    ...
    switch (device) {
    case AudioSystem::DEVICE_OUT_EARPIECE:
//        return device_out_RCV; //we don't have RCV
    case AudioSystem::DEVICE_OUT_SPEAKER:
...
//wenpin.cui
//If inputstream become active, it will hold hw/in/out lock, close pcm_out and reopen it.
//If we get called during this period, input will become abnormal.
//So we require hw lock before requiring in lock.
//If successful, it means inputstream has finished its work, we are safely to get called.
    ...
    if (mHardware->mode() != AudioSystem::MODE_IN_CALL) {
        ...
        for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt){
            ...
            if (NULL != mRouteCtl) {
                ...
                TRACE_DRIVER_OUT
                LOGV("write() wakeup setting %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
            } else {
                LOGE("failed get mixer control");
            }
        }
        ...
        for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt){
            ...
            if (NULL != mRouteCtl) {
                ...
                for (unsigned int idx = 0 ; mixer_ctl_get_num_values(mRouteCtl) > idx ; ++idx)
                    mixer_ctl_set_value(mRouteCtl, idx, mixer[cnt].val);
                TRACE_DRIVER_OUT
                LOGV("read() wakeup setting %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
            } else {
                LOGE("failed get mixer control");
            }
        }
